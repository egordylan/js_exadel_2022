...
You need to iterate through the array and print
in the console only users who registered on
...
function registrData(array){
  let arrayData = [];
  for (let i = 0; i <= users.length; i++) {
    let data = users[i];
      for (let key in data) {
        if (data[key] === '09.10.2021' || data[key] === '10.10.2021') {
          arrayData.push(data);
        }
      }
    }
